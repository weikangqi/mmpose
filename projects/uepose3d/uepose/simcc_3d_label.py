...
class StereoSimCC3DLabel(BaseKeypointCodec):
    ...
    def encode(self,
        ...
        num_ins,_ =keypoints_visible.shape
        lifting_target = [None for _ in range(num_ins)]
        root_z = self.root_z
        with_z_label = False
        if keypoints_3d is not None:
            lifting_target = keypoints_3d.copy()
            root_z = keypoints_3d[..., self.root_index, 2].mean(1).reshape(-1,1)
            ...
            # with_z_label = [True for i in range(num_ins)]
        else:
            ...
            # with_z_label = [False for i in range(num_ins)]
