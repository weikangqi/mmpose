...
class StereoBasePoseEstimator(BaseModel, metaclass=ABCMeta):
    """Base class for pose estimators.
    ...
    def _forward(self,
        ...
        x,y = self.extract_feat(inputs['left'],inputs=['right'])
        if self.with_head:
            x = self.head.forward(x,y)

        return x

    def extract_feat(self, input1: Tensor,input2:Tensor) -> Tuple[Tensor]:
        """Extract features.
        ...
        Returns:
            ...
            resolutions.
        """
        x,y = self.backbone(input1,input2)
        if self.with_neck:
            x = self.neck(x,y)
